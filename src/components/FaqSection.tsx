...
function FaqItem({ q, a, isOpen, onClick }: { q: string; a: string; isOpen: boolean; onClick: () => void }) {
  return (
    <motion.div
      layout
      onClick={onClick}
      className="group cursor-pointer border border-zinc-200 rounded-2xl p-6 hover:border-[#06B6D4]/30 hover:shadow-md hover:shadow-[#06B6D4]/5 transition-all duration-300"
    >
      <div className="flex items-center justify-between gap-4">
        <h3 className="text-base sm:text-lg font-semibold text-[#0a0a1a] group-hover:text-[#06B6D4] transition-colors">
          {q}
        </h3>
        <motion.div
          animate={{ rotate: isOpen ? 45 : 0 }}
          transition={{ duration: 0.2 }}
          className="w-8 h-8 rounded-lg bg-zinc-100 flex items-center justify-center flex-shrink-0 group-hover:bg-[#06B6D4]/10 transition-colors"
        >
          <svg className="w-4 h-4 text-zinc-600 group-hover:text-[#06B6D4]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function FaqSection() {
  ...
  return (
    <section className="relative bg-[#FAFAF9] py-16 md:py-24 lg:py-32 overflow-hidden content-visual-auto">
      <LiquidBlob color="#06B6D4" size={350} className="top-0 left-0 opacity-40" speed={11} />
      <LiquidBlob color="#2DD4BF" size={280} className="bottom-0 right-0 opacity-30" speed={8} delay={3} />
